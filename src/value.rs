...
impl<B:Backend> Value<B>{//TODO scalars
	...
	pub fn zip(self)->Self{// TODO unzip
		fn build<B:Backend>(input:Vec<Value<B>>)->Value<B>{
			if input.len()==0{return input.into()}
			if !input.iter().all(Value::is_multi){return input.into()}

			let len=input.iter().map(Value::len).max().unwrap();
			let mut iters:Vec<_>=input.into_iter().map(Value::into_iter).collect();
			(0..len).map(|_|build(iters.iter_mut().map(|v|v.next().unwrap_or_default()).collect())).collect()
		}
